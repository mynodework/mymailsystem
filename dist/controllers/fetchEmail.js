    ...
    function FetchController() {
        ...
        return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = FetchController.__proto__ || Object.getPrototypeOf(FetchController)).call.apply(_ref, [this].concat(args))), _this), _this.fetch = function (req, res, next) {
            ...
            _this._db.Tag.findAll({ where: { type: "Default" } }).then(function (default_tag) {
                _emailprocess2.default.fetchEmail(page, tag_id, limit, type, keyword, selected, default_id, default_tag, req.email, is_attach, req.user).then(function (data, message) {
            ...
            _MailProvider2.default.changeUnreadStatus(req.checkBody, req.body, req.getValidationResult()).then(function () {
                ...
                _emailprocess2.default.assignMultiple(tag_id, req.body, req.email, req).then(function (data) {
                    console.log(data);
            ...
            _emailprocess2.default.mailAttachment(req.params.mongo_id, req.email, req.history).then(function (result) {
                _this.handleSuccessResponse(req, res, next, result);
            }).catch(_this.handleErrorResponse.bind(null, res));
            ...
            _inbox2.default.fetchEmail(req.email, req.emailLogs, req.spamBox, req.history, 'apiCall').then(function (data) {
            ...
            if (req.body.tag_id) {
                ...
                _this.handleErrorResponse(res, "NO tag id found", next);
            }
        ...
        }, _this.tracking = function (req, res, next) {
            req.emailTrack.update({ tracking_id: req.params.tracking_id, $or: [{ seen: false }, { seen: { $exists: false } }] }, { seen: true, view_time: new Date() }).then(function (response) {
                console.log(response);
            });
        }, _this.fetchTrackingData = function (req, res, next) {
            _emailprocess2.default.fetchTrackingData(req).then(function (response) {
                _this.handleSuccessResponse(req, res, next, response);
            });
        }, _this.sendEmailToNotviewed = function (req, res, next) {
            _emailprocess2.default.sendEmailToNotviewed(req).then(function (response) {
                console.log(response);
                res.json(response);
            });
        }, _this.addNewCandidate = function (req, res, next) {
            _MailProvider2.default.addNewCandidate(req.checkBody, req.body, req.getValidationResult()).then(function (body) {
                req.email.create(body).then(function (response) {
                    _this.handleSuccessResponse(req, res, next, response);
                });
            }, function (err) {
                _this.handleErrorResponse(res, err, next);
            });
        }, _this.assignAnInterviewee = function (req, res, next) {
            _emailprocess2.default.assignAnInterviewee(req).then(function (response) {
                _this.handleSuccessResponse(req, res, next, response);
            });
        }, _this.getCandidateByInterviewee = function (req, res, next) {
            _emailprocess2.default.getCandidateByInterviewee(req).then(function (response) {
                _this.handleSuccessResponse(req, res, next, response);
            });
        }, _this.deleteCampaign = function (req, res, next) {
            _emailprocess2.default.deleteCampaign(req).then(function (response) {
                _this.handleSuccessResponse(req, res, next, response);
            });
        }, _this.starEmail = function (req, res, next) {
            _emailprocess2.default.starEmail(req).then(function (response) {
                _this.handleSuccessResponse(req, res, next, response);
            }).catch(_this.handleErrorResponse.bind(null, res));
        }, _this.getStaredEmails = function (req, res, next) {
            _emailprocess2.default.getStaredEmails(req.email, req.user.id).then(function (response) {
                _this.handleSuccessResponse(req, res, next, response);
            }).catch(_this.handleErrorResponse.bind(null, res));
        }, _this.candidateArchive = function (req, res, next) {
            _emailprocess2.default.candidateArchive(req).then(function (response) {
                _this.handleSuccessResponse(req, res, next, response);
            }).catch(_this.handleErrorResponse.bind(null, res));

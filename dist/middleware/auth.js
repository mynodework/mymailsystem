...
var AuthController = exports.AuthController = function () {
    ...
    _createClass(AuthController, [{
        ...
        key: "requiresInterviewee",
        value: function requiresInterviewee(req, res, next) {
            var token = req.param("accessToken");
            if (token) {
                _jsonwebtoken2.default.verify(token, "secret_key", function (err, docs) {
                    if (err) {
                        ...
                        if (loginTime > endTime) {
                            ...
                            _db2.default.User.find({
                                where: {
                                    id: req.token,
                                    $or: [{ user_type: (0, _constant2.default)().userType.Interviewee }]
                                }
                            }).then(function (user) {
                                if (user) {
                                    req.user = user;
        ...
        key: "requiresAdminHrInterviewee",
        value: function requiresAdminHrInterviewee(req, res, next) {
            var token = req.param("accessToken");
            if (token) {
                _jsonwebtoken2.default.verify(token, "secret_key", function (err, docs) {
                    if (err) {
                        ...
                        if (loginTime > endTime) {
                            req.token = docs.token;
                            _db2.default.User.findOne({ where: { id: req.token, $or: [{ user_type: (0, _constant2.default)().userType.Interviewee }, { user_type: (0, _constant2.default)().userType.hr }, { user_type: (0, _constant2.default)().userType.admin }] } }).then(function (user) {
                                if (user) {
                                    req.user = user;
                                    next();
                                } else {

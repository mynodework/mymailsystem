...
export class AuthController {
    ...
    requiresInterviewee(req, res, next) {
        var token = req.param("accessToken");
        if (token) {
            jwt.verify(token, "secret_key", (err, docs) => {
                if (err) {
                    ...
                    if (loginTime > endTime) {
                        ...
                        db.User.find({
                                where: {
                                    id: req.token,
                                    $or: [{ user_type: tag().userType.Interviewee }]
                                }
                            })
                            .then((user) => {
                                if (user) {
    ...
    requiresAdminHrInterviewee(req, res, next) {
        const token = req.param("accessToken");
        if (token) {
            jwt.verify(token, "secret_key", (err, docs) => {
                if (err) {
                    ...
                    if (loginTime > endTime) {
                        req.token = docs.token;
                        db.User.findOne({where:{id:req.token,$or: [{ user_type: tag().userType.Interviewee },{ user_type: tag().userType.hr },{ user_type: tag().userType.admin }]}})
                            .then((user) => {
                                if (user) {
                                    req.user = user;
                                    next();

...
var sequelize = new Sequelize("", config.db.username, config.db.password, { port: 3306, host: config.db.host });

sequelize.query(`CREATE DATABASE IF NOT EXISTS ${config.db.name}`).then(()=>{
    sequelize = new Sequelize(config.db.name, config.db.username, config.db.password, { port: 3306, host: config.db.host });
    fs
